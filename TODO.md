...
- [x] **`spinbox` widget**: numeric input with up/down buttons (climate control, keypad). Implemented in `src/renderer/widgets/spinbox.ts` with the `lv_spinbox_updatevalue` format (conditional sign on negative ranges, zero-padded to `digits`, decimal point inserted at `digits - decimal_places`). Schema in `src/editor/schema/spinbox.ts` exposes `value`, `range_from`/`to`, `digits`, `decimal_places`, `selected_digit`, `rollover`, `text_align` (the modern ESPHome validator rejects `step` — it derives from `selected_digit`). Still out: focus-gated cursor (LV_PART_CURSOR), increment/decrement actions, `selected`/`scrollbar`/`textarea_placeholder` parts.
